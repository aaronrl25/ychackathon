...
function App() {
  ...
  return <div className="site-shell">
    ...
    <main id="top">
      ...
      <section className="statement"><div className="statement-line"/><img className="statement-phoenix left" src="/assets/phoenix.png" alt=""/><div className="statement-copy"><p>AI should adapt to <em>you.</em></p><p className="dim">Not the other way around.</p></div><img className="statement-phoenix right" src="/assets/phoenix.png" alt=""/></section>
      ...
      <section className="how-section" id="how"><div className="how-copy"><div className="section-kicker">HOW IT WORKS</div><h2>Your habits become<br/><em>your advantage.</em></h2><p>Temper learns quietly in the background while every important decision stays visible and reversible.</p><img className="how-phoenix" src="/assets/phoenix.png" alt=""/></div><div className="steps">{steps.map(([title,text],i) => <div className="step" key={title}><span>0{i+1}</span><div><h3>{title}</h3><p>{text}</p></div>{i < 2 && <div className="step-line"/>}</div>)}<div className="signal-note"><Zap/>Every signal is timestamped and traceable to the moment you approved it.</div></div></section>
      <section className="privacy" id="privacy"><div className="privacy-badge"><ShieldCheck/></div><div><div className="section-kicker">PRIVATE BY DESIGN</div><h2>Your style. Your data.<br/>Your decision.</h2></div><div className="privacy-copy"><p>Memory should earn your trust. See why every preference was learned, choose what stays, and wipe everything whenever you want.</p><div className="privacy-points"><span><Check/> Transparent memory</span><span><Check/> Granular controls</span><span><Check/> One-click delete</span><span><Check/> Local-first storage</span></div></div></section>
      <section className="faq-section"><div><div className="section-kicker">QUESTIONS</div><h2>The useful details.</h2></div><div className="faqs">{[["What does Temper remember?","Only the working preferences that help your AI respond better—like language choices, architecture patterns, and communication style."],["Does it replace my coding assistant?","No. Temper is a portable preference layer designed to make the tools you already use feel more personal."],["Can I see and edit what it learned?","Yes. You can review, lock, edit, or remove every remembered preference."],["Where is my data stored?","Temper keeps your profile private and lets you control where it is stored." ]].map(([q,a],i) => <button className={`faq ${openFaq===i?'open':''}`} onClick={() => setOpenFaq(openFaq===i?-1:i)} key={q} aria-expanded={openFaq===i}><span>{q}</span><ChevronDown/>{openFaq===i && <p>{a}</p>}</button>)}</div></section>
      <section className="cta" id="cta"><div className="cta-flame"><img src="/assets/phoenix.png" alt=""/></div><div className="section-kicker">BE FIRST IN LINE</div><h2>Build with an AI that<br/><em>already gets you.</em></h2><p>Join the early access list and help shape the memory layer for modern software teams.</p><form onSubmit={e=>e.preventDefault()}><label className="sr-only" htmlFor="email">Work email</label><input id="email" type="email" placeholder="you@company.com" required/><button>Request access <ArrowRight size={17}/></button></form><small>Thoughtful updates only. No noise.</small></section>
    </main>
